...
if($tuit != ""){
        ...
        if($retuit != ""){
                $consulta = $con->query( "select retuit from teacherAccount WHERE user = '$name' and retuit is not null");
                
                if ($consulta->num_rows == 0){
                        $salida = array();
                        $salida =  $consulta->fetch_all(MYSQLI_ASSOC);
                        $salida = $retuit;
                        $consulta = $con->query( "UPDATE teacherAccount SET retuit = "."'[". $salida ."]' WHERE user = '$name'");
                        echo "Ready";
                }
                else{
                        $salida = array();
                        ...
                        $consulta = $con->query( "UPDATE teacherAccount SET retuit = "."'[".implode(",", $salida)."]' WHERE user = '$name'");
                        echo "Ready";
                }
